...
### file%3A%2F%2F%2Fhome%2Fshehroz%2FDesktop%2FNew%2520Folder%25203%2FVermithor%2Fsrc%2Fmain%2Fscala%2Fcore%2Ftopcore.scala:82: error: `identifier` expected but `}` found
}
^
...
class topcore extends  Module {
  ...
  val wb_module = Module(new wb)
    ...
    when(decode_module.io.decode_instruction(6,0) === "h67".U){   //jalr
     fetch_module.io.jump_fetch := 1.B
     decode_module.io.decode_regf_wdata := fetch_module.io.pc_fetch_out + 4.U
      fetch_module.io.pc_imm_execute := decode_module.io.ecode_regf_rdata1 + decode_module.io.
}
